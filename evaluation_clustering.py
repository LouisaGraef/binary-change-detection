...
def evaluate_clustering(dataset, cleaned_gold, filter_minus_one_nodes, bc_min_max = [1,3]):
    ...
    if cleaned_gold == True:
        parameter_grid_path = f'./parameter_grids/cleaned/{ds}/'
    else:
        parameter_grid_path = f'./parameter_grids/{ds}/'
    for p in Path(parameter_grid_path).glob('*/parameter_grid.tsv'):
        ...
        for model_name, model_df in model_groups:
            ...
            #print(model_df['GC_pred'])
            #print(model_df['GC_gold'])
            #print(spearman)
            model_df['BC_F1'] = f1
    ...
    class ARI_Precomputed(BaseEstimator):
        ...
        def score(self, X, y):
            ...
            #assert len(df_clean_model_top) == len(X)       # TODO: remove comment
