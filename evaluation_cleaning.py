...
def evaluate_cleaning(dataset, dwug_de_sense):
    ...
    df_filtered = df_cleaning[(df_cleaning['lemma'] == 'Armenhaus') & (df_cleaning['model'].str.startswith('stdnode_0.0915'))]
    print(df_filtered)
    ...
    """
    print(df_cleaning)
    df_cleaning2 = pd.read_pickle("analyze_semeval_de1_df_cleaning.pkl")    # gold
    #params = sorted(set(df['threshold']))
    #pd.set_option('display.max_rows', None)
    #df_filtered = df_cleaning[(df_cleaning['lemma'] == 'Armenhaus') & (df_cleaning['model'].str.startswith('stdnode_0.0915'))]

    df_cleaning_filtered = df_cleaning[(df_cleaning['model'].str.startswith('stdnode_'))]
    print(df_cleaning_filtered)

    df_cleaning_filtered2 = df_cleaning2[(df_cleaning2['model'].str.startswith('stdnode_'))]    # gold
    print(df_cleaning_filtered2)

    p = df_cleaning.groupby('strategy')['threshold'].apply(lambda x: sorted(set(x))).to_dict()
    params = df_cleaning2.groupby('strategy')['threshold'].apply(lambda x: sorted(set(x))).to_dict()
    print(p)
    print(params)

    print(sorted(map(int, params['degreeremove'])))
    print(sorted(map(int, p['dgrnode'])))

    print(sorted(map(float, params['stdnode'])))
    print(sorted(map(float, p['stdnode'])))

    if sorted(map(float, params['stdnode'])) == sorted(map(float, p['stdnode'])):
        print('same')

    lemma_set_gold = set(df_cleaning2['lemma'])
    lemma_set = set(df_cleaning['lemma'])
    print(lemma_set_gold)
    print(lemma_set)
    if lemma_set == lemma_set_gold:
        print("equ")

    lemma_counts_gold = df_cleaning2['lemma'].value_counts().to_dict()
    lemma_counts = df_cleaning['lemma'].value_counts().to_dict()
    print(lemma_counts_gold)
    print(lemma_counts)
    # stdnode filtert bei mir zu viele Knoten raus! 

    df_cleaning_filtered3 = df_cleaning2[(df_cleaning2['lemma'] == 'Ohrwurm') & (df_cleaning2['model'] == 'stdnode_0.0')]  # gold
    df_cleaning_filtered4 = df_cleaning[(df_cleaning['lemma'] == 'Ohrwurm') & (df_cleaning['model'] == 'stdnode_0.0')]
    not_filtered = list(set(df_cleaning_filtered4['identifier']) - set(df_cleaning_filtered3['identifier']))
    print(df_cleaning_filtered3.shape)    # gold
    print(df_cleaning_filtered4.shape)
    print(not_filtered)
    duplicates = df_cleaning_filtered3[df_cleaning_filtered3.duplicated()]      # gold
    print(duplicates)
    duplicates = df_cleaning_filtered4[df_cleaning_filtered4.duplicated()]
    print(duplicates)
    print(df_cleaning_filtered3)        # gold
    print(df_cleaning_filtered4)
    print(df_cleaning_filtered3.shape)        # gold
    print(df_cleaning_filtered4.shape)
    quit()
    """
    ...
    for model in groups_model.keys():                   
        ...
        for lemma in groups_merged_lemma.keys():
            ...
            if lemma == 'Ohrwurm':
                print(model, len(df_merged_lemma_cluster_clean_labels), len(df_merged_lemma))
    ...
    if "paper_data" in dataset:
        os.makedirs(f'cleaning_results/ari/paper_versions/{ds}/', exist_ok=True)
        g.savefig(f'cleaning_results/ari/paper_versions/{ds}/cleaning_main1.pdf')
    else:
        os.makedirs(f'cleaning_results/ari/{ds}/', exist_ok=True)
        g.savefig(f'cleaning_results/ari/{ds}/cleaning_main1.pdf')
