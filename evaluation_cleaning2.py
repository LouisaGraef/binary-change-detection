...
def evaluate_cleaning2(dataset):
    ...
    for model in groups_model.keys():                   
        ...
        for lemma in groups_merged_lemma.keys():
            ...
            cleaned_graph_cluster_stats = get_cluster_stats(graph, threshold=2.5)     
            ...
            results.append({'model':model, 'lemma':lemma, 'win_min_max_normalized':win_min_max_normalized,
                            'conflicts_normalized': conflicts_normalized, 'cluster_changes_left_percentage':cluster_changes_left_percentage,
            ...
            for resample in range(15):
                ...
                results_random.append({'model':model, 'lemma':lemma, 'win_min_max_normalized':win_min_max_normalized,
                            'conflicts_normalized': conflicts_normalized, 'cluster_changes_left_percentage':np.nan,
                                'sense_changes_left_percentage':np.nan, 'clusters_left_percentage':np.nan, 'uses_left_percentage':np.nan, 
                                'clusters_cleaned':np.nan, 'senses_cleaned':np.nan, 'uses_original':np.nan, 
                                'uses_original_all':np.nan, 'uses_cleaned':uses_cleaned, 'uses_removed':np.nan, 
                                'strategy':strategy, 'threshold': threshold, 'resample': resample})
    ...
    df_results_cleaning_mean = df_results_cleaning.groupby(['model']).agg('mean', numeric_only=True).reset_index().sort_values(by='conflicts_normalized', ascending=False)
    df_results_cleaning_random_mean = df_results_cleaning_random.groupby(['model']).agg('mean', numeric_only=True).reset_index().sort_values(by='conflicts_normalized', ascending=False)
    #display(df_results_cleaning_mean)     
    df_results_cleaning_std = df_results_cleaning.groupby(['model']).agg('std', numeric_only=True).reset_index().sort_values(by='conflicts_normalized', ascending=False)
    df_results_cleaning_random_std = df_results_cleaning_random.groupby(['model']).agg('std', numeric_only=True).reset_index().sort_values(by='conflicts_normalized', ascending=False)
    ...
    reference_result_conflicts_normalized = reference_result['conflicts_normalized_mean'].to_list()[0]
    ...
    for strategy in groups_strategy.keys():
        ...
        ax1.set_ylabel('conflicts_normalized (mean)') 
        #ax1.errorbar(df_strategy['threshold_mean'], df_strategy['ARI_mean'], fmt='-', label=strategy, yerr=df_strategy['ARI_std'], color = 'black')
        ax1.errorbar(df_strategy['threshold_mean'], df_strategy['conflicts_normalized_mean'], fmt='-', label=strategy, color = 'black')
        ax1.errorbar(df_strategy_random['threshold_mean'], df_strategy_random['conflicts_normalized_mean'], fmt='-', label='random baseline', yerr=df_strategy_random['conflicts_normalized_std'], color = random_baseline_color)
        ...
        ax1.axhline(y=reference_result_conflicts_normalized, color='gray', linestyle='-', label='no cleaning')    
        ax1.legend()    
        #ax2.legend(loc=0)
        os.makedirs('results', exist_ok=True)
        plt.savefig('results/{0}-{1}x{2}.png'.format(strategy,'threshold','conflicts_normalized_mean'))
        plt.show() 
        plt.close() 
        
        # Plot conflicts_normalized versus number of nodes left
        fig, ax1 = plt.subplots() 

        ax1.set_xlabel('% nodes left') 
        ax1.set_ylabel('conflicts_normalized (mean)') 
        ax1.errorbar(df_strategy['uses_left_percentage_mean'], df_strategy['conflicts_normalized_mean'], fmt='-', label=strategy, color = 'black')
        ax1.errorbar(df_strategy_random['uses_left_percentage_mean'], df_strategy_random['conflicts_normalized_mean'], fmt='-', label='random baseline', yerr=df_strategy_random['conflicts_normalized_std'], color = random_baseline_color)
        ax1.tick_params(axis ='y') 
        ax1.set_ylim(0.0,1.02)
        ax1.legend()
        plt.savefig('results/{0}-{1}x{2}.png'.format(strategy,'uses_left_percentage_mean','conflicts_normalized_mean'))
        #plt.show() 
        plt.close() 
        ...
        ax1.set_ylabel('conflicts_normalized (mean)') 
        ax1.errorbar(df_strategy['clusters_cleaned_mean'], df_strategy['conflicts_normalized_mean'], fmt='-', label=strategy, color = 'black')
        ax1.errorbar(df_strategy_random['clusters_cleaned_mean'], df_strategy_random['conflicts_normalized_mean'], fmt='-', label='random baseline', yerr=df_strategy_random['conflicts_normalized_std'], color = random_baseline_color)
        ax1.tick_params(axis ='y') 
        ax1.set_ylim(0.0,1.02)
        ax1.legend()
        plt.savefig('results/{0}-{1}x{2}.png'.format(strategy,'clusters_cleaned_mean','conflicts_normalized_mean'))
        plt.show() 
        plt.close() 
        """
    ...
    """
    Get conflicts_normalized of one lemma.
    """
    def get_conflicts_normalized(lemma, identifiers):
        with open(f'{dataset}/graphs/opt/{lemma}', 'rb') as f:
                graph = pickle.load(f)
    ...
    def get_perlemma_stats(df, original_stats=None):
        ...
        l2stat = df.groupby('lemma').apply(lambda r: 
                            pd.DataFrame({
                                ...
                                'conflicts_normalized':get_conflicts_normalized(r.name, r.identifier), 
                                #'ri':rand_score(r.label, r.cluster),
                                #'(1-ari)':1-adjusted_rand_score(r.label, r.cluster), 
                                '(1-conflicts_normalized)':1-get_conflicts_normalized(r.name, r.identifier), 
    ...
    with sns.plotting_context('poster'):
        g = sns.relplot(data=plotdf, 
                    x='Δnuses/nuses', y='conflicts_normalized', hue='strategy', style='strategy',
    ...
    #     g.map_dataframe(sns.scatterplot, 'Δnuses/nuses','ari','strategy', hue_order=strategy_order[:1])
        ...
        if "paper_data" in dataset:
            os.makedirs(f'cleaning_results/paper_versions/{ds}/', exist_ok=True)
            g.savefig(f'cleaning_results/paper_versions/{ds}/cleaning_individual_conflicts_normalized.pdf')
        else:
            os.makedirs(f'cleaning_results/{ds}/', exist_ok=True)
            g.savefig(f'cleaning_results/{ds}/cleaning_individual_conflicts_normalized.pdf')
    ...
    mpdf = plotdf.groupby(['model']).agg({'strategy':'first','ntargets':'sum','nuses':'sum', 'nclusters':'sum'}|
                                {c:'mean' for c in plotdf.columns if 'conflicts_normalized' in c or 'Δ' in c})
    ...
    g = plot_mpdf(mpdf, y='Δ(1-conflicts_normalized)/(1-conflicts_normalized)')
    sns.move_legend(g, 'lower center', bbox_to_anchor=(0.4, 1), frameon=True, ncol=2)
    if "paper_data" in dataset:
        os.makedirs(f'cleaning_results/paper_versions/{ds}/', exist_ok=True)
        g.savefig(f'cleaning_results/paper_versions/{ds}/cleaning_main1.pdf')
    else:
        os.makedirs(f'cleaning_results/{ds}/', exist_ok=True)
        g.savefig(f'cleaning_results/{ds}/cleaning_main1.pdf')
    ...
    if "paper_data" in dataset:
        os.makedirs(f'cleaning_results/paper_versions/{ds}/', exist_ok=True)
        g.savefig(f'cleaning_results/paper_versions/{ds}/app_nclusters_avg.pdf')
    else:
        os.makedirs(f'cleaning_results/{ds}/', exist_ok=True)
        g.savefig(f'cleaning_results/{ds}/app_nclusters_avg.pdf')
